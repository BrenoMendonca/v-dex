...
export async function identifyPokemonFromImage(base64Image, mimeType) {
  const fastResult = await runIdentification(base64Image, mimeType, MODEL);

  if (fastResult.identified && fastResult.confidence >= CONFIDENCE_THRESHOLD) {
    return fastResult;
  }

  // O modelo rápido/barato falhou ou ficou em dúvida — provavelmente uma carta full art,
  // dourada ou com brilho de holográfico. Tenta de novo com um modelo mais forte antes de
  // desistir; esse custo extra de latência só é pago nas cartas difíceis.
  try {
