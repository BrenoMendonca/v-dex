...
async function buildEnrichment(pokeApiId) {
  const [data, species] = await Promise.all([
    fetchFromPokeApi(pokeApiId),
  ...
  if (species?.evolution_chain?.url) {
    try {
      ...
      if (response.ok) {
        evolutionChain = parseEvolutionChain(await response.json());
      }
    } catch (error) {
...
export async function getPokemonByNameOrId(query) {
  ...
  if (cached) {
    if (!cached.enrichedAt) {
      ...
      // sobrecarregado etc.) mesmo com o resto do enriquecimento já concluído — tenta de novo
      // só o que ainda falta, sem refazer as outras buscas (habilidades, evolução...) já bem-sucedidas.
      let needsSave = false;
      if (cached.flavorText && !cached.flavorTextPt) {
        cached.flavorTextPt = await translateSafe(cached.flavorText, "descrição");
        needsSave = needsSave || Boolean(cached.flavorTextPt);
      }
      if (cached.genus && !cached.genusPt) {
        cached.genusPt = await translateSafe(cached.genus, "categoria");
        needsSave = needsSave || Boolean(cached.genusPt);
      }
      if (needsSave) {
        await cached.save();
      }
  ...
  const enrichment = await buildEnrichment(data.id);
