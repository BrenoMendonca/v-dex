...
export async function getPokemonByNameOrId(query) {
  ...
  if (cached) {
    if (!cached.enrichedAt) {
      ...
      const docId = cached._id;
      const flavorText = cached.flavorText;
      const genus = cached.genus;
      const needsFlavor = Boolean(flavorText) && !cached.flavorTextPt;
      const needsGenus = Boolean(genus) && !cached.genusPt;

      const runBackfill = async () => {
        try {
          const update = {};
          if (needsFlavor) {
            const flavorTextPt = await translateSafe(flavorText, "descrição");
            if (flavorTextPt) update.flavorTextPt = flavorTextPt;
          }
          if (needsGenus) {
            const genusPt = await translateSafe(genus, "categoria");
          ...
          if (Object.keys(update).length > 0) {
            ...
            // por campo não corre esse risco, e é exatamente o que essa escrita em background precisa.
            await Pokemon.updateOne({ _id: docId }, { $set: update });
          }
        } catch (error) {
      ...
      try {
        ...
        // after() só funciona dentro de uma requisição real do Next.js — chamado de um
        // script/rota de seed fora desse contexto, roda direto (sem problema, ali já se
        // quer esperar a tradução terminar mesmo).
        await runBackfill();
      }
    }
